...
DirectionID Collision::CheckCollision(DynamicObject *_DynamicObject, CGameObject * _Object)
{
	float moveX, moveY;
	...
	if(AABB(ConvertToBox(_DynamicObject->getBound()), ConvertToBox(_Object->getBound()), moveX, moveY) == false)
	{
		...
		if(AABB(_DynamicObject->GetBroadPhaseBox(velocity), ConvertToBox(_Object->getBound()), moveX, moveY) == true)
		{
			...
			{
				if(abs(m_MoveX) >= abs(_DynamicObject->getPhysic()->getVelocity().x * CGameTime::getInstance()->getElapsedGameTime().getMilliseconds() * timeCollision + normalX))
				{
					m_MoveX = _DynamicObject->getPhysic()->getVelocity().x * CGameTime::getInstance()->getElapsedGameTime().getMilliseconds() * timeCollision + normalX;
				}

				if(abs(m_MoveY) >= abs(_DynamicObject->getPhysic()->getVelocity().y * CGameTime::getInstance()->getElapsedGameTime().getMilliseconds() * timeCollision))
				{
					m_MoveY = _DynamicObject->getPhysic()->getVelocity().y * CGameTime::getInstance()->getElapsedGameTime().getMilliseconds() * timeCollision;
				}
				...
				if(normalY != 0.0f)
				{
					if(moveY != 0)
					{
						if(normalY == 1.0f)
						{
							return DirectionID::DIR_TOP;
						}
						else if(normalY == -1.0f)
						{
							return DirectionID::DIR_BOTTOM;
						}
					}
				}
				else
				{
					if(moveX != 0.0f)
					{
						if(normalX == 1.0f)
						{
							return DirectionID::DIR_RIGHT;
						}
						else if(normalX == -1.0f)
						{
							return DirectionID::DIR_LEFT;
						}
					}
				}
			}
